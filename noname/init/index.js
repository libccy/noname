...
export function canUseHttpProtocol() {
	...
	if (typeof nonameInitialized == "string") {
		...
		if (window.cordova) {
			...
			// 由理版判断
			// @ts-ignore
			if (window.noname_yuriInterfaces) {
				return true;
			}
		}
		// 电脑端
...
export function sendUpdate() {
	...
	if (window.cordova) {
		...
		// 由理版判断
		// @ts-ignore
		if (window.noname_yuriInterfaces && typeof window.noname_yuriInterfaces.sendUpdate === 'function') {
			// 给由理版apk的java层传递升级完成的信息
			// @ts-ignore
			return window.noname_yuriInterfaces.sendUpdate() + "?sendUpdate=true";
		}
	}
	// 电脑端
	else if (typeof window.require == "function" && typeof window.process == "object") {
