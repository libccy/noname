...
export class Get {
	 ...
	 * 获取（此牌的）应变条件
	 */
	static yingbianConditions(card) {
	...
	static sort(arr, method, arg) {
		if (method == "seat") return arr.sortBySeat(arg);
	}
	static sortSeat(arr, target) {
	...
	static id() {
		return `${Math.floor(1000000 + 9000000 * Math.random())}${10 + lib.status.globalId++}`;
	}
	...
	static infoTargets(infos) {
		return Array.from(infos || [], info => game.playerMap[info]);
	}
	static cardInfo(card) {
		return [card.suit, card.number, card.name, card.nature];
	}
	static cardsInfo(cards) {
		return Array.from(cards || [], get.cardInfo);
	}
	...
	static infoCards(infos) {
		return Array.from(infos || [], get.infoCard);
	}
	static cardInfoOL(card) {
	...
	static cardsInfoOL(cards) {
		return Array.from(cards || [], get.cardInfoOL);
	}
	static infoCardsOL(infos) {
		return Array.from(infos || [], get.infoCardOL);
	}
	static playerInfoOL(player) {
		return "_noname_player:" + player.playerid;
	}
	static infoPlayerOL(info) {
		return lib.playerOL ? (lib.playerOL[info.slice(15)] || info) : info;
	}
	static playersInfoOL(players) {
		return Array.from(players || [], get.playerInfoOL);
	}
	static infoPlayersOL(infos) {
		return Array.from(infos || [], get.infoPlayerOL);
	}
	...
	static judge(card) {
		return card.viewAs ? lib.card[card.viewAs].judge : get.info(card).judge;
	}
	static judge2(card) {
