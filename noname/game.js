...
export class Game {
	...
	// Stratagem
	// 谋攻
	static setStratagemBuffCost(cardName, cost) {
		game.broadcastAll((clientCardName, clientCost) => lib.stratagemBuff.cost.set(clientCardName, clientCost), cardName, cost);
	}
	static setStratagemBuffEffect(cardName, effect) {
		game.broadcastAll((clientCardName, clientEffect) => lib.stratagemBuff.cost.set(clientCardName, clientEffect), cardName, effect);
	}
	static setStratagemBuffPrompt(cardName, prompt) {
		game.broadcastAll((clientCardName, clientPrompt) => lib.stratagemBuff.cost.set(clientCardName, clientPrompt), cardName, prompt);
	}
	/**
	 * 添加新的属性杀
	 */
	...
	static setYingbianConditionColor(yingbianCondition, color) {
		game.broadcastAll((yingbianCondition, color) => lib.yingbian.condition.color.set(yingbianCondition, color), yingbianCondition, color);
	}
	static setComplexYingbianCondition(yingbianCondition, condition) {
		game.broadcastAll((yingbianCondition, condition) => lib.yingbian.condition.complex.set(yingbianCondition, condition), yingbianCondition, condition);
	}
	static setSimpleYingbianCondition(yingbianCondition, condition) {
		game.broadcastAll((yingbianCondition, condition) => lib.yingbian.condition.simple.set(yingbianCondition, condition), yingbianCondition, condition);
	}
	static setYingbianEffect(yingbianEffect, effect) {
		game.broadcastAll((yingbianEffect, effect) => lib.yingbian.effect.set(yingbianEffect, effect), yingbianEffect, effect);
	}
	static setYingbianPrompt(yingbian, prompt) {
		game.broadcastAll((yingbian, prompt) => lib.yingbian.prompt.set(yingbian, prompt), yingbian, prompt);
	}
	 ...
	 * 用给定的BPM、节拍和偏移生成谱面
	 */
	static generateBeatmapTimeleap(bpm, beats, offset) {
	 ...
	 */
	static parseSkillAudio(skill, player) {
	...
	static trySkillAudio(skill, player, directaudio, nobroadcast/*,index*/) {
	...
	static saveConfigValue(key) {
		game.saveConfig(key, lib.config[key]);
	}
	static saveExtensionConfig(extension, key, value) {
		game.saveConfig(`extension_${extension}_${key}`, value);
	}
	static saveExtensionConfigValue(extension, key) {
		game.saveExtensionConfig(extension, key, game.getExtensionConfig(extension, key));
	}
	static getExtensionConfig(extension, key) {
	...
	static hasPlayer(func, includeOut) {
		return game.players.some(value => (includeOut || !value.isOut()) && func(value));
	}
	static hasPlayer2(func, includeOut) {
