			...
			init(version, config, banned_info) {
				if (lib.node.banned.includes(banned_info)) {
					...
					if (!config.nickname) {
						...
						// 没有系统提示的接口喵？
						game.log("玩家 ", `#y${encodeURIComponent(config.nickname)}`, " 进入房间观战");
						game.me.chat(`玩家 <span style="font-weight: bold; color: rgb(126, 180, 255)">${encodeURIComponent(config.nickname)}</span> 进入房间观战`);
						if (!ui.removeObserve) {
