...
game.import('character',function(lib,game,ui,get,ai,_status){
	return {
		...
		character:{
		    ...
		    olshen_lvbu:['male','shen',5,['baonu','wumou','ol_wuqian','ol_shenfen'],['qun']],
		},
		characterIntro:{
			 ...
			 ol_shenfen:{
				...
				content:function(){
				 ...
				 player.syncStorage('baonu');
					...
					player.line(event.targets,'black');
					"step 2"
					...
					if(event.targets2.length){
						...
						event.redo();
									...
									"step 4"
       player.turnOver();  
       player.storage.ol_shenfen=true;
				},
				ai:{
					order:10,
					result:{
			...
			ol_wuqian:{
				...
				derivation:'wushuang',	
			 	...
			 	content:function(){
							"step 0"
             player.storage.ol_shenfen=false;
							player.storage.baonu-=2;
							player.syncStorage('baonu');
		   			"step 1"		   	
   		   	target.addTempSkill('navEquip2');
		   	 target.markSkillCharacter('navEquip2',player,'装备','防具无效');
			 		  "step 2"
...
if((player.storage.baonu>7||player.storage.ol_shenfen)&&(player.countCards('h',{name:'sha'})||player.countCards('h',{name:'juedou'}))) return -5;
							if(!target.getCards('e',{subtype:['equip2']}).length&&!target.hasSkill("bazhen")) return 0;
if((player.canUse({name:'sha'},target)&&player.countCards('h',{name:'sha'}))||(player.countCards('h',{name:'wanjian'})&&(target.getEquip('tengjia')||target.getEquip('bagua')))){
           if(player.storage.ol_shenfen) return -3.75;
              return -2.5;
              }
							return 0;
						}
            ...
            init:function(player,skill){
              if(!player.getEquip(2))return ;
              var info = lib.card[player.getEquip(2).name];
              if(player.storage.navEquip2){
                lib.card[player.storage.navEquip2.card.name].onLose = player.storage.navEquip2.lose;
                delete player.storage.navEquip2;
              }
              if(typeof info.onLose == 'function'){
                player.storage.navEquip2 = {
                  card:player.getEquip(2),
                  lose:info.onLose,
                };
                info.onLose = null;
              }
              ...
              for(var i=0;i<skills.length;i++){
                if(lib.card[player.getEquip(2).name].skills.contains(skills[i]))continue;
                skills.splice(i--,1);
              }
              player.disableSkill(skill,skills);
            },
            onremove:function(player,skill){
              var skills = player.disabledSkills;
              if(player.storage.navEquip2) {
                lib.card[player.storage.navEquip2.card.name].onLose = player.storage.navEquip2.lose;
              }
              for(var i in skills){
                if(!skills[i].contains(skill))continue;
                if(!player.getEquip(2)){
                  delete skills[i];
                } else if(!lib.card[player.getEquip(2).name].skills.contains(skills[i])){
                  delete skills[i];
                }
              }
              player.enableSkill(skill);
              player.unmarkSkill('navEquip2');
            },
            trigger:{player:['loseEnd','equipAfter']},
            filter:function(event,player){
              if(event.name=='lose'){
                for(var i=0;i<event.cards.length;i++){
                  if(get.position(event.cards[i])=='e'&&get.subtype(event.cards[i])=='equip2'){
                    if(player.storage.navEquip2) {
                      lib.card[player.storage.navEquip2.card.name].onLose = player.storage.navEquip2.lose;
                      delete player.storage.navEquip2;
                    }
                ...
                lib.skill.navEquip2.init(player,'navEquip2');
              }
              return false;
            },
            direct:true,
...
>>>>>>> f0ffe0d256b6b804a130740339a50835b2b6437d
		...
		translate:{
			...
			wuqian_info:'出牌阶段，你可以弃置两枚暴怒标记并获得技能【无双】直到回合结束',
	       	ol_shenfen:'神愤',		
            ol_shenfen_info:'出牌阶段，你可以弃置6枚暴怒标记，对场上所有其他角色造成一点伤害，然后令其弃置4张牌，并将武将牌翻面，每回合限一次',
            wumou:'无谋',	 
            wumou_info:'锁定技:当你使用非延时锦囊牌时，你须选择一项:1，弃置一枚暴怒标记；2，受到一点伤害。',
            ol_wuqian:'无前',		
            ol_wuqian_info:'出牌阶段，你可以弃2枚暴怒标记并选择一名其他角色，你视为拥有技能“无双”并令其防具无效，直到回合结束。',
            wuhun3:'武魂',		
