...
character.yijiang={
	...
	skill:{
		...
		jiaozhao:{
			...
			intro:{
				content:function(storage,player){
					if(player.storage.jiaozhao2){
						return '出牌阶段限一次，你可以展示一张手牌，然后你声明一张基本牌或通常锦囊牌的牌名。在此出牌阶段内，你可以将此手牌当声明的牌使用且你不能被选择为目标';
					}
					else{
						return '出牌阶段限一次，你可以展示一张手牌，然后选择距离最近的一名其他角色，该角色声明一张基本牌或通常锦囊牌的牌名。在此出牌阶段内，你可以将此手牌当声明的牌使用且你不能被选择为目标';
					}
				}
			},
			...
			content:function(){
				...
				if(player.storage.jiaozhao1&&player.storage.jiaozhao2){
					...
					if(player.storage.jiaozhao1){
						...
						prompt='摸一张牌或更改矫诏的描述<br><br><div class="text">更改描述：将“基本牌”改为“基本牌或通常锦囊牌”';
					}
		...
		shifei:{
			...
			check:function(event,player){
				...
				var players=game.filterPlayer();
				for(var i=0;i<players.length;i++){
					if(players[i].num('h')>nh){
						if(!player.hasShan()||ai.get.attitude(player,players[i])<=0) return true;
					}
				}
				return false;
			},
					...
					player:function(player){
						...
						for(var i=0;i<players.length;i++){
							if(player!=players[i]&&ai.get.damageEffect(players[i],player,players[i],'fire')<0){
								var att=ai.get.attitude(player,players[i]);
								if(att>0){
									num-=Math.max(1,players[i].num('e'));
								}
								else if(att<0){
									num+=Math.max(1,players[i].num('e'));
								}
	...
	translate:{
		...
		taoluan5:'滔乱',
		taoluan_backup:'滔乱',
		taoluan_info:'出牌阶段，你可视为使用任意一张基本牌或通常锦囊牌（此牌不得是本局游戏你以此法使用过的牌），然后你令一名其他角色选择一项：1.交给你一张与你以此法使用的牌类别相同的牌；2.你失去1点体力且滔乱无效直到回合结束',
		jiaozhao:'矫诏',
		jiaozhao2:'矫诏',
		jiaozhao_info:'出牌阶段限一次，你可以展示一张手牌，然后选择距离最近的一名其他角色，该角色声明一张基本牌的牌名。在此出牌阶段内，你可以将此手牌当声明的牌使用且你不能被选择为目标',
		danxin:'殚心',
		danxin_info:'当你受到伤害后，你可以摸一张牌，或对“矫诏”的描述依次执行下列一项修改：1.将“基本牌”改为“基本牌或通常锦囊牌”；2.将“选择距离最近的一名其他角色，该角色”改为“你”',
		duliang:'督粮',
		duliang2:'督粮',
		...
		chanhui:'谮毁',
		chanhui_info:'出牌阶段限一次，当你使用【杀】或黑色通常锦囊牌指定唯一目标时，你可令可以成为此牌目标的另一名其他角色选择一项：交给你一张牌并成为此牌的使用者;或成为此牌的额外目标。',
		jiaojin:'骄矜',
		...
		qiaoshui:'巧说',
		qiaoshui_info:'出牌阶段开始时，你可与一名其他角色拼点。若你赢，你使用的下一张基本牌或通常锦囊牌可以额外指定任意一名其他角色为目标或减少指定一个目标；若你没赢，你不能使用锦囊牌直到回合结束',
		junxing:'峻刑',
		...
		mieji_info:'你使用黑色通常锦囊牌仅指定一个目标后，可以额外指定一个目标',
		...
		wuyan_info:'锁定技，你使用的通常锦囊牌对其他角色无效;其他角色使用的通常锦囊牌对你无效。',
		...
		zhichi_info:'锁定技，你的回合外，你每受到一次伤害，任何【杀】或通常锦囊牌均对你无效，直到该回合结束。',
		zhichi2_info:'智迟已发动',
