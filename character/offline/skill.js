...
const skills = {
	scls_luoshen: {
		...
		async content(event, trigger, player) {
			player.addTempSkill("scls_luoshen_add");
			while (true) {
				...
				if (judge > 0) {
					...
					bool = await player
						...
						.set("frequentSkill", "scls_luoshen")
						.forResultBool();
				}
				if (!bool) {
			...
			player.addMark("scls_luoshen_add", cards.length);
		},
		subSkill: {
			add: {
				mod: {
					maxHandcard(player, num) {
						return num + player.countMark("scls_luoshen_add");
					},
				},
				intro: {
					content: "手牌上限+#"
	...
	scls_zishu: {
		audio: "zishu",
		locked: true,
		group: ["scls_zishu_draw", "scls_zishu_discard", "scls_zishu_mark"],
		subSkill: {
			draw: {
				trigger: {
					player: "gainAfter",
				...
				filter(event, player) {
					...
					return event.getParent(2).name != "scls_zishu_draw";
				},
				async content(event, trigger, player) {
			...
			mark: {
				...
				silent: true,
				async content(event, trigger, player) {
					if (trigger.name === "gain") trigger.gaintag.add("scls_zishu");
					else player.removeGaintag("scls_zishu");
				},
			},
		},
		ai: {
			...
			skillTagFilter(player, tag, arg) {
				if (tag === "nogain") return player !== _status.currentPhase;
			},
		},
	},
	scls_yingyuan: {
		...
		filter(event, player) {
			...
			return (type === "basic" || type === "trick") && !player.hasHistory("custom", evt => {
				return evt.scls_yingyuan_name === type;
			});
		},
		async cost(event, trigger, player) {
			event.result = await player
				.chooseTarget(
					...
					"令一名其他角色获得" + get.translation(trigger.card),
					lib.filter.notMe
				)
				.set("ai", target => {
		...
		async content(event, trigger, player) {
			...
			await event.targets[0].gain(trigger.card, "log", "gain2");
			player.getHistory("custom").push({ scls_yingyuan_name: type });
		},
	},
	scls_yinshi: {
