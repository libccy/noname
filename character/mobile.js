		...
		skill:{
			...
			xinwanlan:{
				...
				check:function(event,player){
					...
					if(player.countCards('hs',card=>player.canSave(card,target))>=1+event.num-event.player.hp) return false;
			...
			wanlan:{
				...
				check:function(event,player){
					if(get.attitude(player,event.player)<4) return false;
					if(player.countCards('hs',card=>player.canSave(card,target))>=1-event.player.hp) return false;
					if(event.player==player||event.player==get.zhu(player)) return true;
