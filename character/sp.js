	...
	return {
		...
		skill:{
			...
			olniluan:{
				...
				direct:true,
				filter:function(event,player){
					var hs=player.getCards('he'),target=event.player,bool=false; 
					for(var i of hs){
						if(game.checkMod(i,player,'unchanged','cardEnabled2',player)!==false&&player.canUse(get.autoViewAs({name:'sha'},[i]),target,false)&&get.color(i,player)=='black'){
							bool=true;
							break;
						}
					}
				...
				content:function(){
					'step 0'
					player.chooseCard(get.prompt('olniluan'),'he',function(card,player){
		...
		translate:{
			...
			ol_hejin:'OL何进',
                        ...
                        olcuorui:'挫锐',
                        olcuorui_info:'①锁定技，游戏开始时，你将手牌摸至X张（X为游戏人数）。②锁定技，当你成为延时锦囊牌的目标后，你跳过下个判定阶段。',
                        olniluan:'逆乱',
