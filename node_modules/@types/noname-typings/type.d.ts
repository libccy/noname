...
interface When {
    ...
    filter(fun: (event: GameEvent, player: Player) => boolean): When
    /**
     * 向临时技能中添加技能可以拥有的一切属性
     *  */
    assign(obj: Skill): When
    /**与filter相反，用于移除一个筛选条件 */
    removeFilter(fun: (event: GameEvent, player: Player) => boolean): When
    /**添加临时技能的充分筛选条件，只要有一个添加的筛选条件通过时，就会触发 */
    filter2(fun: (event: Event, player: Player) => boolean): When
    /**与filter2相反，用于移除一个筛选条件 */
    removeFilter2(fun: (event: Event, player: Player) => boolean): When
    /**触发时，弹出的显示提示字样 */
