...
declare type Player = import('../../../noname/library/element/player.js').Player;
...
//汇总的所有的卡的名字
type ALLCardName = "caoyao" | "dinvxuanshuang" | "du" | "fengyinzhidan" | "hufu" | "jiu" | "sha" | "shan" | "shenmiguo" |
...
//汇总的所有的主动发出的时机信号，即由event.trigger()发出的
type triggerSignal = 'addShownCardsAfter' | 'hideShownCardsAfter' | 'executeDelayCardEffect' | 'gift' | 'giftDeny' |
    'giftDenied' | 'giftAccept' | 'giftAccepted' | 'recastingLose' | 'recastingLost' | 'recast' | 'recastingGain' |
    'recastingGained' | 'showCharacterEnd' | 'showCharacterAfter' | 'zhuUpdate' | 'washCard' | 'phaseBeginStart' | 'subPlayerDie' |
    'triggerHidden' | 'triggerInvisible' | 'triggerAfter' | 'phaseJudge' | "phaseDrawBegin1" | "phaseDrawBegin2" | 'phaseDiscard' |
    'compareCardShowBefore' | 'compare' | "rewriteDiscardResult" | "rewriteGainResult" | 'useCard1' | 'yingbian' | 'useCard2' | 'useCard' |
    'useSkill' | 'discard' | 'loseToDiscardpile' | 'respond' | 'damageBegin1' | 'damageBegin2' | 'damageBegin3' | 'damageBegin4' |
    'damageZero' | 'damage' | 'damageSource' | 'changeHp' | 'dying' | 'die' | 'judge' | 'judgeFixing' | 'removeSubPlayer' | 'roundStart' |
    'useCardToOmitted' | 'useCardToEnd' | 'useCardToAfter' | 'useCardToBefore' | 'useCardToBegin' | /** 一般不会自动触该改时机 */'enterGame' |
    'useCardToPlayer' | 'useCardToTarget' | 'useCardToPlayered' | 'useCardToTargeted' | 'useCardToExcluded' | 'logSkill' | 'logSkillBegin' |
    'phaseZhunbei' | 'phaseJieshu' |
    'shaHit' | 'shaMiss' | 'shaDamage' | 'shaUnhirt' | 'shaDamage' | 'shaMiss' | 'shaDamage' | 'shaUnhirt' | 'wuguRemained' | 'juedou' | 'eventNeutralized' |
    'gameStart' | 'phaseBeforeStart' | 'phaseBeforeEnd' | 'phaseBeginStart' | 'phaseChange'

//汇总所有的事件，即由game.createEvent()创建的事件
type triggerEvent = 'yingbianZhuzhan' | /** 卡的名字对应一个事件，例如'sha'事件 */`${ALLCardName}` | 'replaceEquip' |
/** 装备某个卡事件，例如'equip_baiyin'*/`equip_${ALLCardName} ` |
    'debateCallback' | /** 每个技能对应一个事件 */'【skill】' |
    'name_Cancel' | 'pre_【skill】' | 'compareMultiple' | `${ALLCardName}_ContentBefore` | `${ALLCardName}_ContentAfter` |
    `lose_${ALLCardName}` |
    '【skill】_ContentBefore' | '【skill】_ContentAfter' | '_save' |
    'executeDelayCardEffect' | 'gift' | 'recast' | 'disableEquip' | 'enableEquip' | 'expandEquip' | 'chooseToDebate' | 'chooseCooperationFor' |
    'chooseToPlayBeatmap' | 'chooseToMove' | 'chooseToGuanxing' | 'changeGroup' | 'chooseToDuiben' | 'chooseToPSS' |
    'chooseToEnable' | 'chooseToDisable' | 'swapEquip' | 'disableJudge' | 'enableJudge' | 'phase' | 'phaseZhunbei' | 'phaseJudge' |
    'phaseDraw' | 'phaseUse' | 'phaseDiscard' | 'phaseJieshu' | 'chooseToUse' | 'chooseToRespond' | 'chooseToDiscard' | 'chooseToCompare' |
    'chooseSkill' | 'discoverCard' | 'chooseButton' | 'chooseButtonOL' | 'chooseCardOL' | 'chooseCard' | 'chooseUseTarget' | 'chooseTarget' |
    'chooseCardTarget' | 'chooseControl' | 'chooseBool' | 'choosePlayerCard' | 'discardPlayerCard' | 'gainPlayerCard' |
    'showHandcards' | 'showCards' | 'viewCards' | 'moveCard' | 'useCard' | 'useSkill' | 'draw' | 'discard' | 'loseToDiscardpile' | 'respond' |
    'gain' | 'addToExpansion' | 'lose' | 'damage' | 'recover' | 'doubleDraw' | 'loseHp' | 'loseMaxHp' |
    'gainMaxHp' | 'changeHp' | 'changeHujia' | 'dying' | 'die' | 'equip' | 'addJudge' | 'judge' | 'turnOver' | 'link' |
    'callSubPlayer' | 'toggleSubPlayer' | 'exitSubPlayer' | 'arrangeTrigger' | 'yingbianEffect' | 'loseAsync' |
    'cardsDiscard' | 'cardsGotoOrdering' | 'cardsGotoSpecial' | 'cardsGotoPile' | 'replaceHandcards' |
    'finish_game' | 'loadPackage' | 'phaseLoop' | 'gameDraw' | 'chooseCharacter' |
    'baiyin_recover' | 'taipingyaoshu'
//所有由事件产生的时机信号，每个事件对应5个信号
type triggerEventSinal = `${triggerEvent}Before` | `${triggerEvent}Begin` | `${triggerEvent}End` | `${triggerEvent}After` | `${triggerEvent}Skipped`
//所有的信号
type signal = triggerEventSinal | triggerSignal
