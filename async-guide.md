...
由于在`1.10.13`版本中，诗笺喵已经修正了写法，现在你在async写法中不再需要`game.pause`了

上述代码在`1.10.13`版本中只需要这样写:
...
当然，你为了兼容旧版本，你也可以继续使用`game.pause`，这在新版本中只是没有效果，但并不会造成错误或者其他影响
...
而在async写法中，你可以直接在里面访问到你外层定义的变量，这是`step`写法不具备的优势
...
另外使用async写法的技能出现错误时，不会出现报错信息不正确的问题，async写法同样也是因为没有二次编译，不会丢失错误信息，增加你的错误排查效率
